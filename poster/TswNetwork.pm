...
$IP = $ENV{'REMOTE_ADDR'};
$FIP = $ENV{'HTTP_X_FORWARDED_FOR'};
$Client = $IP ? "tsweb,".$IP : 'tsweb-text';
$Client = $Client."<-".$FIP if $FIP;
...
sub setblockChannel 
{
	if ($^O =~ /MSWin32/)
	{
		## windows does not implement fcntl, so we will 
		## just ignore blocking
		return 1;
	}
    my ($C, $v) = @_;
    $C = getChannel ($C);
...
sub recvChannel {
    my $C = &openChannel (shift);
    my $buff = undef;
    my $res = recv ($C->{'sock'}, $buff, 65536, 0);
